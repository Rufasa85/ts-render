...
class Feedback {
  ...
  constructor(email: string,feedbackType:string,feedback:string, feedback_id: string) {
    this.email = email;
...
class FeedbackService {
  ...
  async addFeedback(email:string, feedbackType:string = "Complaint",feedback: string) {
    if (!email || !feedback) {
      throw new Error('Feedback and email cannot be blank');
    }

    // Add a unique id to the feedback using uuid package
    const newFeedback: Feedback = { email: email,feedbackType:feedbackType,feedback:feedback, feedback_id: uuidv4() };
